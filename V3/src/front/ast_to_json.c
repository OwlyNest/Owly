...
void create_json(Node *node) {
    ...
    FILE *json_file = fopen("out/ast.json", "w");
    if (json_file) {
...
cJSON *func_decl_to_json(Node *node) {
	...
	if (node->func_decl.arg_count != 0) {
		...
		cJSON_AddStringToObject(args_obj, "args", "void");
	}
...
cJSON *expr_to_json(Expr *expr) {
	...
	switch (expr->kind) {
		...
		case EXPR_LITERAL: {
			...
			cJSON_AddStringToObject(val_obj, "raw", expr->literal->raw);
		...
		case EXPR_TERNARY: {
			cJSON *type_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(type_obj, "type", "TERNARY");
			cJSON_AddItemToArray(ex, type_obj);
			cJSON *cond_obj = cJSON_CreateObject();
			cJSON_AddItemToObject(cond_obj, "cond", expr_to_json(expr->ternary.cond));
			cJSON_AddItemToArray(ex, cond_obj);
			cJSON *true_obj = cJSON_CreateObject();
			cJSON_AddItemToObject(true_obj, "true", expr_to_json(expr->ternary.true_expr));
			cJSON_AddItemToArray(ex, true_obj);
			cJSON *false_obj = cJSON_CreateObject();
			cJSON_AddItemToObject(false_obj, "false", expr_to_json(expr->ternary.false_expr));
			cJSON_AddItemToArray(ex, false_obj);
			break;
		}

		case EXPR_MEMBER: {
			cJSON *type_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(type_obj, "type", "MEMBER");
			cJSON_AddItemToArray(ex, type_obj);
			cJSON *obj_obj = cJSON_CreateObject();
			cJSON_AddItemToObject(obj_obj, "object", expr_to_json(expr->member.object));
			cJSON_AddItemToArray(ex, obj_obj);
			cJSON *member_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(member_obj, "member", expr->member.member);
			cJSON_AddItemToArray(ex, member_obj);
			cJSON *arrow_obj = cJSON_CreateObject();
			cJSON_AddNumberToObject(arrow_obj, "is arrow", expr->member.is_arrow);
			cJSON_AddItemToArray(ex, arrow_obj);
			cJSON *off_obj = cJSON_CreateObject();
			cJSON_AddNumberToObject(off_obj, "offset", expr->member.offset);
			cJSON_AddItemToArray(ex, off_obj);
			break;
		}

		case EXPR_SIZEOF: {
			cJSON *type_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(type_obj, "type", "SIZEOF");
			cJSON_AddItemToArray(ex, type_obj);
			if (expr->sizeof_expr.expr) {
				cJSON *expr_obj = cJSON_CreateObject();
				cJSON_AddItemToObject(expr_obj, "Expression", expr_to_json(expr->sizeof_expr.expr));
				cJSON_AddItemToArray(ex, expr_obj);
			}
			if (expr->sizeof_expr.type && expr->sizeof_expr.type->type == NODE_TYPE) {
				cJSON *typen_obj = cJSON_CreateObject();
				cJSON_AddItemToObject(typen_obj, "type", type_node_to_json(expr->sizeof_expr.type));
				cJSON_AddItemToArray(ex, typen_obj);
			}
			cJSON *istype_obj = cJSON_CreateObject();
			cJSON_AddNumberToObject(istype_obj, "istype", expr->sizeof_expr.is_type);
			cJSON_AddItemToArray(ex, istype_obj);
			break;
		}

		case EXPR_CAST: {
			cJSON *type_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(type_obj, "type", "CAST");
			cJSON_AddItemToArray(ex, type_obj);
			cJSON *target = cJSON_CreateObject();
    		cJSON_AddItemToObject(target, "target_type", type_node_to_json(expr->cast.target_type));
			cJSON_AddItemToArray(ex, target);
			cJSON *expr_obj = cJSON_CreateObject();
    		cJSON_AddItemToObject(expr_obj, "expr", expr_to_json(expr->cast.expr));
			cJSON_AddItemToArray(ex, expr_obj);
    		break;
		}
		...
		case EXPR_SET: {
			cJSON *type_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(type_obj, "type", "SET");
			cJSON_AddItemToArray(ex, type_obj);

			cJSON *elements_obj = cJSON_CreateObject();
			cJSON *elements_arr = cJSON_CreateArray();
			for (size_t i = 0; i < expr->set.element_count; i++) {
				cJSON_AddItemToArray(elements_arr, expr_to_json(expr->set.elements[i]));
			}
			cJSON_AddItemToObject(elements_obj, "elements", elements_arr);
			cJSON_AddItemToObject(ex, "elements", elements_obj);
			break;
		}

		case EXPR_INDEX: {
			cJSON *type_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(type_obj, "type", "INDEX");
			cJSON_AddItemToArray(ex, type_obj);

			cJSON *idx_obj = cJSON_CreateObject();
			cJSON_AddItemToObject(idx_obj, "idx", expr_to_json(expr->index.index));
			cJSON_AddItemToArray(ex, idx_obj);

			cJSON *arr_obj = cJSON_CreateObject();
			cJSON_AddStringToObject(arr_obj, "array name", expr->index.array->identifier);
			cJSON_AddItemToArray(ex, arr_obj);
			break;
		}
	}
	cJSON *wrapper = cJSON_CreateObject();
...
cJSON *array_to_json(Node *node) {
	cJSON *array_arr = cJSON_CreateArray();

	cJSON *type_obj = cJSON_CreateObject();
	cJSON_AddItemToObject(type_obj, "type", type_node_to_json(node->array.type));
	cJSON_AddItemToArray(array_arr, type_obj);

	cJSON *name_obj = cJSON_CreateObject();
	cJSON_AddStringToObject(name_obj, "name", node->array.name);
	cJSON_AddItemToArray(array_arr, name_obj);

	cJSON *dim_obj = cJSON_CreateObject();
	cJSON *dim_arr = cJSON_CreateArray();
	for (size_t i = 0; i < node->array.dim_count; i++) {
		cJSON_AddItemToArray(dim_arr, cJSON_CreateNumber(node->array.dim[i]));
	}
	cJSON_AddItemToObject(dim_obj, "Dimensions", dim_arr);
	cJSON_AddItemToArray(array_arr, dim_obj);

	cJSON *val_obj = cJSON_CreateObject();
	if (node->array.value) {
		cJSON_AddItemToObject(val_obj, "value", expr_to_json(node->array.value));
	} else {
		cJSON_AddStringToObject(val_obj, "value", "<uninitialised>");
	}
	cJSON_AddItemToArray(array_arr, val_obj);

	cJSON *wrapper = cJSON_CreateObject();
    cJSON_AddItemToObject(wrapper, "ARRAY", array_arr);
	return wrapper;
}